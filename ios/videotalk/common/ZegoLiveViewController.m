...
- (void)viewDidLoad {
    ...
    self.beautifyFeature = [ZegoSetting sharedInstance].beautifyFeature;
    self.filter = [ZegoSetting sharedInstance].filterFeature;
    ...
    self.videoSize = [ZegoSetting sharedInstance].avConfig.videoEncodeResolution;
...
- (void)setAnchorConfig:(UIView *)publishView
{
    ...
    int ret = [[ZegoManager api] setAVConfig:config];
    assert(ret);
    
    bool b = [[ZegoManager api] setFrontCam:self.useFrontCamera];
    assert(b);
    
    b = [[ZegoManager api] enableMic:self.enableMicrophone];
    assert(b);
    
    b = [[ZegoManager api] enableBeautifying:self.beautifyFeature];
    assert(b);
    
    b = [[ZegoManager api] setFilter:self.filter];
    assert(b);
    
    [self enablePreview:self.enablePreview LocalView:publishView];
    [[ZegoManager api] setPreviewViewMode:self.viewMode];
...
{
    ZegoAnchorOptionViewController *optionController = [[ZegoAnchorOptionViewController alloc] init];
...
// 为播放流的视图添加约束。view 为播放视图，containerView 为播放视图的容器视图，viewCount 为视图的序号（房间内有多个用户连麦时，需要根据这个序号布局）
- (BOOL)setContainerConstraints:(UIView *)view containerView:(UIView *)containerView viewIndex:(NSUInteger)viewIndex
{
...
{
    [[ZegoManager api] respondJoinLiveReq:seq result:(agreed == false)];
}
...
- (void)shareToQQ:(NSString *)hls rtmp:(NSString *)rtmp bizToken:(NSString *)bizToken bizID:(NSString *)bizID streamID:(NSString *)streamID
{
    ...
    NSString *title = @"ZEGO VideoLive";
    NSString *description = @"快来围观我的直播吧！";
...
- (void)auxCallback:(void *)pData dataLen:(int *)pDataLen sampleRate:(int *)pSampleRate channelCount:(int *)pChannelCount
{
    ...
    {
        //初始化auxData
        NSURL *auxURL = [[NSBundle mainBundle] URLForResource:@"music.pcm" withExtension:nil];
        if (auxURL)
        {
...
- (void)handleAudioRouteChanged:(NSNotification *)notification
{
        ...
        reason == AVAudioSessionRouteChangeReasonOverride)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            [[ZegoSetting sharedInstance] checkHeadSet];
            if (![ZegoSetting sharedInstance].useHeadSet && self.enableLoopback)
            {
                self.enableLoopback = NO;
            }
...
- (void)enablePreview:(BOOL)enable LocalView:(UIView *)view
{
    if (enable && view)
    {
        [[ZegoManager api] setPreviewView:view];
...
- (void)enableAudioRecord:(BOOL)enable
{
    ...
    // * test audio record
    //
    [[ZegoManager api] enableAudioRecord:enable];
    if (enable)
    {
        [[ZegoManager api] setAudioRecordDelegate:self];
    }
    else
    {
        [[ZegoManager api] setAudioRecordDelegate:nil];
...
- (void)onSelectedFilter:(NSInteger)row
{
    ...
    [[ZegoManager api] setFilter:(ZegoFilter)row];
    self.filter = row;
}
...
- (void)setEnablePreviewMirror:(BOOL)enablePreviewMirror
{
    ...
    [[ZegoManager api] enablePreviewMirror:enablePreviewMirror];
}

- (void)setEnableCaptureMirror:(BOOL)enableCaptureMirror
{
    _enableCaptureMirror = enableCaptureMirror;
    [[ZegoManager api] enableCaptureMirror:enableCaptureMirror];
}

- (void)setEnableLoopback:(BOOL)enableLoopback
{
    _enableLoopback = enableLoopback;
    [[ZegoManager api] enableLoopback:enableLoopback];
    if (enableLoopback)
        [[ZegoManager api] setLoopbackVolume:50];
}
