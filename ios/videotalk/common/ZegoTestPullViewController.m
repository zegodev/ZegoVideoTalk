...
- (void)clearAllStream
{
    ...
    {
        [[ZegoManager api] stopPlayingStream:info.streamID]; // 停止拉流
...
//    bool ret = [[ZegoDemoHelper api] startPlayingStream:streamID inView:bigView];
    bool ret = [[ZegoManager api] startPlayingStream:self.streamID inView:bigView];
    // 设置观看直播的View的模式
    [[ZegoManager api] setViewMode:ZegoVideoViewModeScaleAspectFill ofStream:self.streamID];
...
- (void)onStreamUpdateForDelete:(NSArray<ZegoStream *> *)streamList
{
    for (ZegoStream *stream in streamList)
    {
        ...
        if (![self isStreamIDExist:streamID])
            continue;
        [[ZegoManager api] stopPlayingStream:streamID]; // 结束拉流
...
- (void)enableAudioRecord:(BOOL)enable {
    if (enable) {
        ...
        config.channels = 1;
        [[ZegoManager api] enableSelectedAudioRecord:config];
...
- (void)onCloseButton:(id)sender
{
    ...
    {
        [[ZegoManager api] logoutRoom]; // 退出房间
    }
